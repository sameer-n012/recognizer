...
def main(input_dir: Path, out_path: Path):
    ...
    for path in tqdm(input_dir.rglob("*"), desc="Indexing files", unit="file"):
        ...
        try:
            rec["sha256"] = sha256_for_file(path)
        except Exception:
            failures += 1
            logger.error(f"Failed to hash file: {path} (ID: {aid})", exc_info=False)
        ...
        if rec["type"] == "video":
            try:
                ...
                rec.update(
                    {"width": None, "height": None, "fps": None, "duration": None, "codec": None}
                )
                logger.error(
                    f"Failed to probe video: {path} (ID: {aid})", exc_info=False
                )
        else:
            try:
                ...
                logger.error(
                    f"Failed to read image: {path} (ID: {aid})", exc_info=False
                )
    ...
    print(f"Indexed {len(df) - failures} assets ({failures} hash failures) -> {out_path}")
    logger.info(
        f"Indexed {len(df) - failures} assets ({failures} hash failures) -> {out_path}"
    )
