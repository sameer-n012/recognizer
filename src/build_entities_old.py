...
def main(index_path, persons_dir, faces_dir, out_dir, min_frames_per_entity):
    ...
    for _, row in tqdm(
        ...
        print()
        # print(row["type"], asset_id, row["path"])
        # print(face_map)
        ...
        logger.info(
            f"Processed asset: {asset_id}, entities created: {len(entity_frames)}"
        )

    print("Entity construction complete")
    logger.info("Entity construction complete")
